      ...
      {isOpen && (
        <div id="add-website-modal" className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={onClose}
            className="absolute inset-0 bg-bg-darkest/80 backdrop-blur-md"
          />
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg rounded-3xl border border-white/10 bg-card-dark/90 p-6 md:p-8 shadow-2xl z-10 overflow-hidden backdrop-blur-xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between pb-4 border-b border-white/5">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-white border border-white/10">
                  <Bookmark className="h-5 w-5 text-accent-text" />
                </div>
                <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full rounded-xl border border-white/5 bg-card-dark py-3 px-4 text-sm font-medium text-white focus:border-white/20 focus:outline-none transition-all duration-200 cursor-pointer"
                    >
                      {categories.map(cat => (
                        <option key={cat.id} value={cat.id} className="bg-card-dark text-white">
                          {cat.name}
                        </option>
                      ))}
                    </select>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 items-center">
                      <input
                        ...
                        className="flex-1 w-full min-w-0 rounded-xl border border-white/5 bg-white/5 py-3 px-4 text-sm font-medium text-white placeholder:text-white/20 focus:border-white/20 focus:outline-none transition-all duration-200"
                      />
                      {logo && (
