...
export default function GreenPixelBg() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // 1. Draw a deep soft radial green glow in the center-right (atmosphere matching the pixel art)
      const gradient = ctx.createRadialGradient(
        width * 0.75, height * 0.7, 10,
        width * 0.75, height * 0.7, Math.max(width * 0.5, 400)
      );
      gradient.addColorStop(0, 'rgba(16, 185, 129, 0.06)'); // Soft emerald glow
      ...
      strands.forEach((strand) => {
        strand.points.forEach((pt) => {
          ...
          if (Math.random() < 0.0001) {
            ctx.fillStyle = `rgba(52, 211, 153, ${currentAlpha * 0.4})`;
