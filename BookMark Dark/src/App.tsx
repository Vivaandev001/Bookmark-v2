...
import { renderUserAvatar } from './components/ProfileAvatar';

const SpotlightCard = React.lazy(() => import('./components/ReactBits/SpotlightCard'));
...
export default function App() {
  ...
  const [avatarId, setAvatarId] = useState('classic-spiky');
  ...
  // Dynamically update data-theme attribute on document root to force dark theme
  useEffect(() => {
  ...
  useEffect(() => {
    // 1. Username and Avatar settings
    ...
    const savedAvatarId = localStorage.getItem('bookmark_avatar_id');
    if (savedAvatarId) {
      setAvatarId(savedAvatarId);
    }
    const savedAvatarColor = localStorage.getItem('bookmark_avatar_color');
    if (savedAvatarColor) {
  ...
  const handleSaveProfile = (name: string, avId: string, avColor: string) => {
    setUsername(name);
    setAvatarId(avId);
    setAvatarColor(avColor);
    localStorage.setItem('bookmark_username', name);
    localStorage.setItem('bookmark_avatar_id', avId);
    localStorage.setItem('bookmark_avatar_color', avColor);
  };
  ...
  return (
    <div id="bookmark-app-root" className="min-h-screen flex flex-col bg-bg-darkest text-white selection:bg-accent-primary/40 selection:text-white">
        ...
        <AnimatePresence mode="wait">
          ...
          {view === 'landing' ? (
            ...
            >
              ...
              {/* High-end Pitch area */}
              <section className="bg-bg-darker/70 py-20 px-6 backdrop-blur-md">
                  ...
                  <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
                    {isDark ? (
                      <React.Suspense fallback={
                        ...
                        <SpotlightCard className="rounded-3xl border border-white/5 bg-card-dark/40 p-8 hover:border-white/10 transition-all duration-300" spotlightColor="rgba(103, 232, 249, 0.12)">
                          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/5 text-white border border-white/10 mb-5">
                            <Terminal className="h-5 w-5 text-accent-text" />
                          </div>
                          <h3 className="font-display text-lg font-bold text-white mb-2">Desktop Environment</h3>
                      ...
                      <div className="rounded-3xl border border-white/5 bg-card-dark/40 p-8 hover:border-white/10 transition-all duration-300">
                        <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/5 text-white border border-white/10 mb-5">
                          <Terminal className="h-5 w-5 text-accent-text" />
                        </div>
                        <h3 className="font-display text-lg font-bold text-white mb-2">Desktop Environment</h3>
                    ...
                    {isDark ? (
                      <React.Suspense fallback={
                        <div className="rounded-3xl border border-white/5 bg-card-dark/40 p-8">
                          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/5 text-white border border-white/10 mb-5">
                            <Compass className="h-5 w-5 text-accent-text" />
                          </div>
                    ...
                    {isDark ? (
                      <React.Suspense fallback={
                        <div className="rounded-3xl border border-white/5 bg-card-dark/40 p-8">
                          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/5 text-white border border-white/10 mb-5">
                            <BookmarkCheck className="h-5 w-5 text-accent-text" />
                          </div>
            ...
            >
              ...
              <div className="absolute inset-0 -z-20 bg-gradient-to-tr from-card-dark via-bg-darker to-bg-darkest overflow-hidden">
                <GreenPixelBg />
              </div>

              {/* Aurora atmosphere on launch page top most side */}
              <div className="absolute top-0 left-0 right-0 h-56 -z-10 opacity-35 pointer-events-none overflow-hidden">
              ...
              <div className="lg:hidden flex items-center justify-between px-6 py-3 bg-bg-darker/95">
                <button
                  onClick={() => setIsMobileSidebarOpen(!isMobileSidebarOpen)}
                  className="flex items-center gap-2 text-xs font-semibold text-white/80 hover:text-white"
                >
                  <Menu className="h-4.5 w-4.5 text-accent-text" />
              ...
              <div className="flex-1 px-6 py-8 md:px-10 md:py-12 overflow-y-auto max-w-7xl mx-auto w-full">
                ...
                <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between pb-4 mb-6">
                  <div>
                    {username && (
                      <div className="mb-2.5 flex items-center gap-2 text-xs font-semibold text-accent-text bg-accent-primary/10 border border-white/5 px-3 py-1 rounded-full w-fit">
                        <Sparkles className="h-3.5 w-3.5 text-accent-text animate-pulse" />
                  ...
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-full text-[10px] font-bold tracking-wide transition-all ${
                      selectedFilter === 'all' || selectedFilter === 'favorites' || selectedFilter === 'recent'
                        ? 'bg-white/10 text-accent-text font-extrabold'
                  ...
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-full text-[10px] font-bold tracking-wide transition-all ${
                      selectedFilter === 'most-used'
                        ? 'bg-accent-primary/20 text-accent-text font-extrabold'
                        : 'bg-transparent text-white/50 hover:text-white hover:bg-white/5'
                    }`}
                  >
                    Most Used
      ...
      <footer id="bookmark-footer" className="bg-bg-darkest py-8 text-center px-6">
        <div className="mx-auto max-w-7xl flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-white/40">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-[10px] font-medium tracking-wider uppercase text-white/30">made by LumiQ</span>
          </div>
        </div>
      </footer>
