...
const BorderGlow = React.lazy(() => import('./ReactBits/BorderGlow'));
...
export default function AppCard({
  ...
  const cardContent = (
    <div className="flex-1 flex flex-col justify-between w-full h-full min-h-[180px] text-left">
      <div>
        {/* Card Header Row */}
        <div className="flex items-start justify-between gap-3 mb-4">
          
          {/* Favicon frame */}
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-bg-darkest border border-white/5 shadow-inner shrink-0 group-hover:border-white/20 transition-all duration-300">
            <img
              ...
              <AnimatePresence>
                {showMenu && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-1 w-48 rounded-xl border border-white/10 bg-card-dark p-1.5 shadow-2xl z-30 backdrop-blur-md"
                  >
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    <div className="relative">
                      ...
                      {showMoveSubmenu && (
                        <div className="mt-1 ml-3 pl-2 border-l border-white/10 flex flex-col gap-1 max-h-40 overflow-y-auto">
                          {categories.map(cat => (
                            <button
                              key={cat.id}
                              type="button"
                              ...
                              className="flex w-full items-center justify-between gap-2 rounded-lg px-2 py-1 text-left text-[11px] font-medium text-white/70 hover:bg-white/5 hover:text-white transition-colors cursor-pointer"
                            >
                              <span className="truncate">{cat.name}</span>
                              {app.category === cat.id && <Check className="h-3 w-3 text-cyan-400" />}
                            </button>
                          ))}
                        </div>
                      )}
        ...
        <div className="mb-2">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <h3 className="font-display text-base font-extrabold text-white tracking-tight line-clamp-1 group-hover:text-accent-text transition-colors">
              {app.name}
            </h3>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-white/5 text-[10px] text-white/40 font-medium">
        ...
        <a
          ...
          className="flex items-center gap-1 text-white hover:text-accent-text transition-colors font-bold"
        >
          <span>Launch</span>
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col justify-between rounded-3xl shadow-2xl overflow-hidden min-h-[220px]"
    >
      {isDark ? (
        <React.Suspense fallback={
          <div className="border border-white/5 bg-card-dark/40 p-5 rounded-3xl h-full w-full flex flex-col justify-between min-h-[220px]">
            {cardContent}
          </div>
        }>
          <BorderGlow
            className="w-full h-full p-5 min-h-[220px]"
            backgroundColor="rgba(20, 16, 15, 0.4)"
            borderRadius={24}
            glowColor="180 80 80"
